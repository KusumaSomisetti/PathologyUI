...
export default function CasesClient({
  initialCaptureViewerOnly = false,
}: {
  initialCaptureViewerOnly?: boolean;
}) {
  const [captureViewerOnly, setCaptureViewerOnly] = useState(initialCaptureViewerOnly);
  ...
  useEffect(() => {
    const syncCaptureMode = () => {
      const params = new URLSearchParams(window.location.search);
      setCaptureViewerOnly(params.get("capture") === "viewer");
    };

    syncCaptureMode();
    window.addEventListener("popstate", syncCaptureMode);
    return () => window.removeEventListener("popstate", syncCaptureMode);
  }, []);
